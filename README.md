...
│               ▼               ▼                ▼                   │
│  ┌────────────────────┐ ┌──────────────┐ ┌───────────────────┐     │
│  │  @aero/core       │ │ @aero/types │ │  @aero/reporter  │     │
│  │   (核心追踪器)      │ │  (类型定义)   │ │    (数据上报)       │     │
│  └────────┬───────────┘ └──────────────┘ └───────────────────┘     │
│           │                                                        │
│           │                                                        │
...
- **具体插件实现**:
  - `@aero/plugins/js-error`: JavaScript 错误捕获
  - `@aero/plugins/network`: 网络请求监控
  - `@aero/plugins/resource`: 资源加载监控
  - `@aero/plugins/performance`: 性能指标监控
  - `@aero/plugins/pv`: 页面访问(PV)监控
  - `@aero/plugins/metric`: 业务指标监控
...
```javascript
// 轻量级引入方式
import { createTracker } from '@aero/core'
import jsErrorPlugin from '@aero/plugins/js-error'
import { Reporter } from '@aero/reporter'
